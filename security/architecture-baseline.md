...
The HTTP API uses ASP.NET Core Identity and short-lived P-256 ECDSA JWT access tokens. Refresh tokens are opaque random values stored in a `Secure`, `HttpOnly`, `SameSite=Strict` host-prefixed browser cookie; only hashes are stored in PostgreSQL.
