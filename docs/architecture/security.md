...
The access token is carried explicitly in the `Authorization` header and is therefore not ambient browser authority. The refresh and logout endpoints are different: they intentionally act on the refresh cookie. `SameSite=Strict` blocks cross-site cookie submission, but same-site sibling origins are still distinct origins and can otherwise trigger simple cookie-authenticated POSTs. The browser therefore adds the non-simple `X-UFWeb-CSRF` request header to credentialed API requests, and `Ufw.Web` requires that header for refresh and logout. Cross-origin JavaScript must pass a CORS preflight before it can send that header, while an HTML form cannot add it.
