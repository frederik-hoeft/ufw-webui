...
Free-text search operates on a dedicated presentation/search projection instead of rendered HTML. Searchable content includes structural rule fields, comments, rule notes, tag names, group names/comments, canonical UFW command text, and compatible known-host aliases. Each filter produces structured match evidence, so the UI can explain that a row matched a note, tag, group, interface, address alias, or command fragment without reimplementing search rules inside Razor.
...
Rule groups are deliberately different from tags. A group has stable UUID identity, a case-insensitive unique name, and an optional comment, and one semantic rule can reference at most one group. Empty groups are first-class and can be created or edited from the dedicated management page. The shared metadata editor can also create a name-only group lazily when a user accepts a previously unknown group name while editing or creating a rule. Active group filters store the stable group ID rather than the mutable name, so a rename can reconcile the displayed filter label without changing its meaning.

The group-management page resolves the group's stored semantic `RuleId` memberships against the current authoritative rule snapshot for presentation. One semantic membership can therefore expand to multiple live occurrences when duplicate rules exist, while a membership that no longer resolves is shown as stale rather than silently discarded. Group deletion does not use that presentation projection as mutation authority: the workflow independently derives the exact occurrence set from the refreshed group catalog and authoritative snapshot before signing.

Deleting a non-empty group is a cross-system workflow owned by a feature service rather than by Razor lifecycle code. The browser refreshes group membership and firewall state, presents the exact members for confirmation, verifies that membership has not changed before mutation, signs one batch-delete intent over the reviewed snapshot occurrence IDs, and interprets the daemon's structured result. The ASP group is deleted only after a fresh catalog read proves it is empty. Partial, stale, or uncertain firewall outcomes keep the group, and a concurrent reassignment after deletion likewise prevents group cleanup. Ordinary single-rule deletion can offer the same empty-only group cleanup when the rule is the sole semantic member and one live occurrence exists.

Known hosts and network interfaces both assist rule authoring, but their authority differs. Known hosts are ASP-owned aliases whose authoring value is always one literal canonical address or network. A literal alias stores that value directly. A DNS-backed alias instead stores its DNS source and last successful resolution time alongside the resolved address; creation or a DNS-configuration change resolves the alias name in its selected IPv4/IPv6 family, and the known-host management page can explicitly reconcile it later. The browser surfaces that source and timestamp so DNS-backed configuration is visible rather than looking like a manually entered address. Expanded rule details reuse the same known-host editor: exact source or destination matches link back to the corresponding alias, while an otherwise-unaliased literal endpoint can seed the create dialog with that address. The rule-details surface does not own a second copy of known-host mutation logic.
