...
UFW does not retain one cross-family evaluation order. IPv4 and IPv6 rules are stored and evaluated in separate rule sets, and `ufw status numbered` presents them as one deterministic sequence by concatenating the IPv4 partition before the IPv6 partition. Relative placement between an IPv4 row and an IPv6 row therefore has no packet-processing meaning; only ordering within one concrete family affects first-match behavior. The browser reflects that model with separate IPv4 and IPv6 rule sections and permits reordering only inside one section. Rows continue to display their combined UFW numbered-status coordinate for exact snapshot correspondence, while move-to-position input is explicitly family-local. The combined numbered sequence remains the canonical snapshot representation used by the existing signed-intent protocol.

A family-neutral add does not create a durable UFW object linking its two materializations. Once listed, the resulting IPv4 and IPv6 rows are ordinary concrete rules and may also be indistinguishable from rules added independently. The application therefore does not infer or display a synthetic pairing between them. Future ASP-owned rule metadata may record explicit authoring provenance when the application itself has such information, but semantic similarity across the two UFW partitions is not sufficient evidence of shared identity.
