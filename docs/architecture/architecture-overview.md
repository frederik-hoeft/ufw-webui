...
| Application state | `Ufw.Web` / PostgreSQL | Users, refresh-token families, known-host aliases and DNS-resolution metadata, interface presentation metadata, and rule notes/tags. These records enrich workflows but cannot make firewall state exist. |
