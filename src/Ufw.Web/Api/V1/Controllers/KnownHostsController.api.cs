...
public sealed partial class KnownHostsController
{
    ...
    /// <summary>
    /// Creates a known-host alias from either a literal IPv4/IPv6 address or a DNS name resolved to one host address.
    /// </summary>
    [HttpPost]
    [ProducesResponseType<KnownHostInventoryResponse>(StatusCodes.Status200OK)]
    ...
    /// <summary>
    /// Updates a known-host alias without allowing its resolved address family to change.
    /// </summary>
    [HttpPut("{id:guid}")]
    [ProducesResponseType<KnownHostInventoryResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    ...
    /// <summary>
    /// Resolves a DNS-backed known host again and replaces its authoring address with the latest address from the configured family.
    /// </summary>
    [HttpPost("{id:guid}/dns/reconcile")]
    [ProducesResponseType<KnownHostInventoryResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status409Conflict)]
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    public partial Task<IActionResult> ReconcileDnsAsync(Guid id, CancellationToken cancellationToken);
