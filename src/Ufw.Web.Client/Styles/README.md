...
`Styles/app.scss` is the single global Sass entry point. Keep only styles that are genuinely global or intentionally integrate with framework-rendered DOM under `Styles/`:
...
- `MyComponent.scss` (or `MyPage.scss`) stays globally compiled through `Styles/app.scss`. Use this when the component intentionally styles MudBlazor-generated descendants,
